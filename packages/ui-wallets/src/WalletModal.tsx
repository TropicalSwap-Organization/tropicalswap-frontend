...
const TabContainer = ({
  ...
  return (
    <AtomBox position="relative" zIndex="modal" className={modalWrapperClass}>
      <AtomBox position="absolute" style={{ left: "12px", top: "-48px" }}>
        <TabMenu
          activeIndex={index}
    ...
    >
          ...
          >
            ...
            >
              <AtomBox
                ...
                style={{ borderRadius: "13px", border: "2px solid #400C8B" }}
              >
                {isImage ? (
            ...
            <Text
              fontSize="12px"
              textAlign="center"
              className="text-[#400C8B] font-mitr text-[14px] leading-[20px]"
            >
              {wallet.title}
            </Text>
          </Button>
      ...
      {!showMore && wallets.length > walletDisplayCount && (
        ...
        >
          ...
          >
            <AtomBox
              className={`${walletIconClass} bg-white`}
              style={{ border: "2px solid #400C8B" }}
              display="flex"
              justifyContent="center"
              alignItems="center"
              bgc="dropdown"
            >
              <MoreHorizontalIcon color="#400C8B" />
            </AtomBox>
            <Text
              fontSize="12px"
              textAlign="center"
      ...
      >
        <AtomBox px="48px">
          <Heading
            color="#C54177"
            as="h4"
            style={{ fontSize: "20px", lineHeight: "22px", color: "#C54177" }}
            className="text-[20px] leading-[22px] font-mitr"
          >
            {t("Connect Wallet")}
          </Heading>
          <Text
            pt="24px"
            pb="32px"
            className="font-mitr text-[#400C8B] text-[14px] leading-[20px]"
          >
            {t(
...
const Intro = ({ docLink, docText }: { docLink: string; docText: string }) => {
  ...
  return (
    <>
      <Heading
        as="h1"
        className="text-[20px] leading-[22px] font-mitr text-[#400C8B]"
      >
        {t("Haven’t got a wallet yet?")}
      </Heading>
      <Image src="/images/wallet-icon.png" width={198} height={178} />
      <Button
        as={LinkExternal}
        /*     color="backgroundAlt"
        variant="subtle"*/
        style={{ background: "#FFBF00", color: "#400C8B" }}
        href={docLink}
        className="font-mitr"
      >
        {docText}
