...
import {
  secondsToWeeks,
  weeksToSeconds,
} from "views/Pools/components/utils/formatSecondsToWeeks";
...
const LockDurationField: React.FC<
  React.PropsWithChildren<LockDurationFieldProps>
> = ({
  duration,
  setDuration,
  isOverMax,
  ...
  const maxAvailableDuration = currentDurationLeft
    ? MAX_LOCK_DURATION - currentDurationLeft
    : MAX_LOCK_DURATION;
  ...
  const currentDurationInWeeks = useMemo(
    () => currentDuration && secondsToWeeks(currentDuration),
    [currentDuration]
  );

  const maxDurationInWeeks = useMemo(
    () => secondsToWeeks(MAX_LOCK_DURATION),
    []
  );
          ...
          {DURATIONS.map((week) => {
            ...
            return (
              <Button
                ...
                style={{
                  color: weekSeconds === duration ? "white" : "white",
                  background: weekSeconds === duration ? "#400C8B" : "#828282",
                  boxShadow: "none",
                }}
              >
                {week}W
      ...
      {currentDurationLeft && currentDurationInWeeks === maxDurationInWeeks ? (
        <Message variant="warning">
          <MessageText maxWidth="240px">
            {t(
              'Recommend choosing "MAX" to renew your staking position in order to keep similar yield boost.'
            )}
          </MessageText>
        </Message>
      ) : null}
