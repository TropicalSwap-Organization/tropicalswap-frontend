...
export default function useYieldBoosterState(
  yieldBoosterStateArgs: UseYieldBoosterStateArgs
) {
  ...
  const { proxyCreated, refreshProxyAddress, proxyAddress } =
  ...
  if (!account || isUndefinedOrNull(locked)) {
    ...
    lockedEnd === "0" ||
    new Date() > new Date(parseInt(lockedEnd) * 1000)
  ) {
