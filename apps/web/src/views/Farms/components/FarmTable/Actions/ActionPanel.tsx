...
import {
  ActionContainer as ActionContainerSection,
  ActionContent,
  ActionTitles,
} from "./styles";
...
const ActionPanel: React.FunctionComponent<
  React.PropsWithChildren<ActionPanelProps>
> = ({
  details,
  apr,
  multiplier,
  ...
  const { proxyFarm, shouldUseProxyFarm } = useContext(
      ...
      <InfoContainer>
        <ValueContainer>
          ...
          {!isDesktop && (
            <>
              <ValueWrapper>
                <Text style={{ color: "#400C8B" }}>{t("APR")}</Text>
                <Apr {...apr} useTooltipText={false} boosted={farm.boosted} />
              </ValueWrapper>
              <ValueWrapper>
                <Text style={{ color: "#400C8B" }}>{t("Multiplier")}</Text>
                <Multiplier {...multiplier} />
              </ValueWrapper>
              <ValueWrapper>
                <Text style={{ color: "#400C8B" }}>{t("Liquidity")}</Text>
                <Liquidity {...liquidity} />
              </ValueWrapper>
            </>
          )}
        ...
        {/*        <Flex mb="2px" justifyContent={alignLinksToRight ? 'flex-end' : 'flex-start'}>
          <StyledLinkExternal href={infoUrl} color="#400C8B" style={{color: "#400C8B"}}>
            {t('See Pair Info')}
          </StyledLinkExternal>
        </Flex>*/}
        <Flex
          ...
          justifyContent={alignLinksToRight ? "flex-end" : "flex-start"}
        >
          <StyledLinkExternal isBscScan href={bsc} style={{ color: "#400C8B" }}>
            {t("View Contract")}
          </StyledLinkExternal>
        </Flex>
      </InfoContainer>
