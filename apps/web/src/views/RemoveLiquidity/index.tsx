...
export default function RemoveLiquidity({
  ...
  const canZapOut = useMemo(
    () => zapSupportedChainId.includes(chainId) && zapMode,
    [chainId, zapMode]
  );
  ...
  const { pair, parsedAmounts, error, tokenToReceive, estimateZapOutAmount } =
    useDerivedBurnInfo(
      currencyA ?? undefined,
      currencyB ?? undefined,
      removalCheckedA,
  ...
  // pair contract
  const pairContractRead: Contract | null = usePairContract(
  ...
  async function onAttemptToApprove() {
    if (!pairContractRead || !pair || !library || !deadline)
  ...
  const onCurrencyAInput = useCallback(
    (value: string): void => onUserInput(Field.CURRENCY_A, value),
    [onUserInput]
  );
  const onCurrencyBInput = useCallback(
    (value: string): void => onUserInput(Field.CURRENCY_B, value),
    [onUserInput]
  );
  ...
  async function onZapOut() {
    if (!chainId || !library || !account || !estimateZapOutAmount)
      throw new Error("missing dependencies");
    ...
    if (
      oneCurrencyIsNative &&
      tokenToReceive.toLowerCase() === WNATIVE[chainId].address.toLowerCase()
    ) {
      ...
      .catch((err) => {
        ...
        setLiquidityState({
          ...
          liquidityErrorMessage:
            err && err?.code !== 4001
              ? `Remove Liquidity failed: ${err.message}`
              : undefined,
          txHash: undefined,
        });
      });
    ...
    if (approval === ApprovalState.APPROVED) {
      ...
      if (oneCurrencyIsNative) {
        ...
        args = [
          ...
          amountsMin[
            currencyBIsNative ? Field.CURRENCY_A : Field.CURRENCY_B
          ].toString(),
          amountsMin[
            currencyBIsNative ? Field.CURRENCY_B : Field.CURRENCY_A
          ].toString(),
          account,
    ...
    for (let i = 0; i < methodNames.length; i++) {
      ...
      try {
        ...
        safeGasEstimate = calculateGasMargin(
          await routerContract.estimateGas[methodNames[i]](...args)
        );
      } catch (e) {
  ...
  const handleSelectCurrencyA = useCallback(
    (currency: Currency) => {
      if (currencyIdB && currencyId(currency) === currencyIdB) {
        router.replace(
          `/remove/${currencyId(currency)}/${currencyIdA}`,
          undefined,
          { shallow: true }
        );
      } else {
        router.replace(
          `/remove/${currencyId(currency)}/${currencyIdB}`,
          undefined,
          { shallow: true }
        );
      }
  ...
  const handleSelectCurrencyB = useCallback(
    (currency: Currency) => {
      if (currencyIdA && currencyId(currency) === currencyIdA) {
        router.replace(
          `/remove/${currencyIdB}/${currencyId(currency)}`,
          undefined,
          { shallow: true }
        );
      } else {
        router.replace(
          `/remove/${currencyIdA}/${currencyId(currency)}`,
          undefined,
          { shallow: true }
        );
      }
    ...
    <Page>
      <AppBody>
        ...
        <CardBody style={{ background: "white" }}>
          <AutoColumn gap="20px">
            <RowBetween>
              <Text style={{ color: "#400C8B" }}>{t("Amount")}</Text>
              <Button
                variant="text"
                scale="sm"
            ...
            {!showDetailed && (
              ...
              >
                <Text
                  ...
                  bold
                  mb="16px"
                  style={{ lineHeight: 1, color: "#400C8B" }}
                >
                ...
                <Flex flexWrap="wrap" justifyContent="space-evenly">
                  <Button
                    ...
                    style={{ background: "#828282", color: "#FFFFFE" }}
                  >
                    25%
                  </Button>
                  <Button
              ...
              <AutoColumn gap="12px">
                ...
                <LightGreyCard>
                  ...
                  >
                    ...
                    <Flex>
                      <Text small bold style={{ color: "#400C8B" }}>
                        {formattedAmounts[Field.CURRENCY_A] || "0"}
                      </Text>
                      <Text small ml="4px" style={{ color: "#400C8B" }}>
                  ...
                  >
                    ...
                    <Flex>
                      <Text bold small style={{ color: "#400C8B" }}>
                  ...
                  {chainId && (oneCurrencyIsWNative || oneCurrencyIsNative) ? (
                    ...
                    >
                      {oneCurrencyIsNative ? (
                        ...
                        >
                          {t("Receive %currency%", {
                            currency: WNATIVE[chainId]?.symbol,
                          })}
                        </StyledInternalLink>
            ...
            <AutoColumn gap="12px" style={{ marginTop: "16px" }}>
              ...
              <LightGreyCard>
                <Flex justifyContent="space-between">
                  <Text small color="textSubtle" style={{ color: "#400C8B" }}>
                    1 {currencyA?.symbol} =
                  </Text>
                  <Text small style={{ color: "#400C8B" }}>
          ...
          <RowBetween mt="16px">
            ...
            <Text bold color="primary" style={{ color: "#400C8B" }}>
              {allowedSlippage / 100}%
            </Text>
          </RowBetween>
          {poolData && (
