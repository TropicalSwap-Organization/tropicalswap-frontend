...
export default function RemoveStableLiquidity({
  ...
  const { pair, parsedAmounts, error } = useStableDerivedBurnInfo(
    currencyA ?? undefined,
    currencyB ?? undefined
  );
  ...
  const { stableSwapConfig, stableSwapContract } =
    useContext(StableConfigContext);

  const [approval, approveCallback] = useApproveCallback(
    parsedAmounts[Field.LIQUIDITY],
    needUnwrapped
      ? nativeHelperContract?.address
    ...
    for (let i = 0; i < methodNames.length; i++) {
      ...
      try {
        ...
        safeGasEstimate = calculateGasMargin(
          await contract.estimateGas[methodNames[i]](...args)
        );
      } catch (e) {
