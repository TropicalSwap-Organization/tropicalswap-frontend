...
function useChainlinkRoundDataSet() {
  ...
  const { data, error } = useSWRMulticall<
...
const ChainlinkChartWrapper = styled(Flex)<{ isMobile?: boolean }>`
  ...
  background: ${({ theme, isMobile }) =>
    isMobile ? theme.card.background : theme.colors.gradientBubblegum};
`;
  ...
  return (
    <PairPriceDisplay
      ...
      value={
        hoverData
          ? hoverData.answer
          : formatBigNumberToFixed(answerAsBigNumber, 4, 8)
      }
  ...
  return (
    <ChainlinkChartWrapper {...props}>
      <FlexGap
        ...
        flexWrap="wrap"
        columnGap="12px"
        height={["56px", undefined, undefined, undefined, "44px"]}
      >
    ...
    <Dot
      ...
      onClick={() => {
        const roundIndex = sortedRounds.findIndex(
          (round) => round.closeOracleId === props.payload.roundId
        );
