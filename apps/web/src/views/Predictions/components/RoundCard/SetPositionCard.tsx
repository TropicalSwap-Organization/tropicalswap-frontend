...
> = ({ position, togglePosition, epoch, onBack, onSuccess }) => {
  ...
  const handleInputChange = (input: string) => {
    ...
    if (inputAsBn.eq(0)) {
      ...
      const percentage = inputAsFn
        .divUnsafe(maxValueAsFn)
        .mulUnsafe(hundredAsFn);
  ...
  const handlePercentChange = useCallback(
    (sliderPercent: number) => {
      if (sliderPercent > 0) {
        const maxValueAsFn = FixedNumber.from(maxBalance);
        const hundredAsFn = FixedNumber.from(100);
        const sliderPercentAsFn = FixedNumber.from(
          sliderPercent.toFixed(18)
        ).divUnsafe(hundredAsFn);
  ...
  const handleEnterPosition = async () => {
    ...
    const args =
      token.symbol === "CAKE" ? [epoch, valueAsBn.toString()] : [epoch];
  ...
  useEffect(() => {
    ...
    const hasSufficientBalance =
      inputAmount.gt(0) && inputAmount.lte(maxBalance);
