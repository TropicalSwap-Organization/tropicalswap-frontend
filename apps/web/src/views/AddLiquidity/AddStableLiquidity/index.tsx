...
export default function AddStableLiquidity({ currencyA, currencyB }) {
  ...
  const minLPOutput = useMemo(
    () =>
      expectedOutputWithoutFee &&
      calculateSlippageAmount(expectedOutputWithoutFee, allowedSlippage)[0],
  ...
  const executionSlippage = useMemo(() => {
    ...
    return ONE_HUNDRED_PERCENT.subtract(
      new Percent(liquidityMinted.quotient, expectedOutputWithoutFee.quotient)
    );
  ...
  const { stableSwapContract, stableSwapConfig } =
    useContext(StableConfigContext);
  ...
  async function onAdd() {
    ...
    const atLeastOneCurrencyProvided =
      parsedAmountA?.greaterThan(0) || parsedAmountB?.greaterThan(0);
    ...
    const lpMintedSlippage = calculateSlippageAmount(
      liquidityMinted,
    ...
    const tokenAmounts =
      stableSwapConfig?.token0?.address ===
    ...
    let args = [
      tokenAmounts,
      minLPOutput?.toString() || lpMintedSlippage?.toString(),
    ];
    ...
    await estimate(...args, value ? { value } : {})
      .then((estimatedGasLimit) =>
        method(...args, {
          ...
          const amountA =
            parsedAmounts[Field.CURRENCY_A]?.toSignificant(3) || "0";
          const symbolB = currencies[Field.CURRENCY_B]?.symbol;
          const amountB =
            parsedAmounts[Field.CURRENCY_B]?.toSignificant(3) || "0";
  ...
  const buttonDisabled =
    !isValid || notApprovalYet || (slippageSeverity > 2 && !expertMode);
            ...
            <AutoColumn gap="20px">
              {noLiquidity && (
                <ColumnCenter>
                  <Message variant="warning">
                    <div>
                      <Text bold mb="8px" style={{ color: "#FFBF00" }}>
                        {t("You are the first liquidity provider.")}
                      </Text>
                      <Text mb="8px" style={{ color: "#FFBF00" }}>
                        {t(
              ...
              {!account ? (
                ...
                <AutoColumn gap="md">
                  ...
                  <CommitButton
                    variant={
                      !isValid || slippageSeverity > 2 ? "danger" : "primary"
                    }
                    onClick={() => {
                      if (expertMode) {
                        onAdd();
