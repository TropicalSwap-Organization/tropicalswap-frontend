    ...
    <ResponsiveGrid>
      <LinkExternal
        ...
        href={getBlockExploreLink(
          transaction.hash,
          "transaction",
          chainName === "ETH" && ChainId.ETHEREUM
        )}
      >
      ...
      <Text style={{ color: "#400C8B" }}>
        <Text style={{ color: "#400C8B" }}>{`${formatAmount(abs0)} ${
          transaction.token0Symbol
        }`}</Text>
      </Text>
      <Text style={{ color: "#400C8B" }}>
        <Text style={{ color: "#400C8B" }}>{`${formatAmount(abs1)} ${
          transaction.token1Symbol
        }`}</Text>
      </Text>
      <LinkExternal
...
> = ({ transactions }) => {
  ...
  const [txFilter, setTxFilter] = useState<TransactionType | undefined>(
    undefined
  );
          ...
          .sort((a, b) => {
            if (a && b) {
              ...
              const [first, second] = toBeAbsList.includes(sortField)
                ? [
                    Math.abs(firstField as number),
                    Math.abs(secondField as number),
                  ]
  ...
  useEffect(() => {
    if (transactions) {
      ...
      if (filteredTransactions.length % ITEMS_PER_INFO_TABLE_PAGE === 0) {
        setMaxPage(
          Math.floor(filteredTransactions.length / ITEMS_PER_INFO_TABLE_PAGE)
        );
      } else {
        setMaxPage(
          Math.floor(filteredTransactions.length / ITEMS_PER_INFO_TABLE_PAGE) +
