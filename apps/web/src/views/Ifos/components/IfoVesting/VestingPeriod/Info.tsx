...
const StyleTag = styled(Tag)<{ isPrivate: boolean }>`
  ...
  background: ${({ theme, isPrivate }) =>
    isPrivate ? theme.colors.gradientBlue : theme.colors.gradientViolet};
`;
...
const Info: React.FC<React.PropsWithChildren<InfoProps>> = ({
  ...
  const labelText =
    poolId === PoolIds.poolUnlimited ? t("Public Sale") : t("Private Sale");
  ...
  useSWRImmutable(
    !isPublicIfoDataInitialized &&
      currentBlock && ["fetchPublicIfoData", currentBlock, data.ifo.id],
    async () => {
  ...
  const releasedAtSaleEnd = useMemo(() => {
    return new BigNumber(offeringAmountInToken).times(
      new BigNumber(1).minus(vestingPercentage)
    );
  }, [offeringAmountInToken, vestingPercentage]);

  const amountReleased = useMemo(() => {
    return new BigNumber(releasedAtSaleEnd)
      .plus(vestingReleased)
      .plus(vestingComputeReleasableAmount);
  }, [releasedAtSaleEnd, vestingReleased, vestingComputeReleasableAmount]);

  const received = useMemo(() => {
    const alreadyClaimed = new BigNumber(releasedAtSaleEnd).plus(
      vestingReleased
    );
    return alreadyClaimed.gt(0)
      ? getFullDisplayBalance(alreadyClaimed, token.decimals, 4)
      : "0";
  ...
  const claimable = useMemo(() => {
    ...
    const claimableAmount = isVestingOver
      ? vestingComputeReleasableAmount.plus(remain)
      : vestingComputeReleasableAmount;
    return claimableAmount.gt(0)
      ? getFullDisplayBalance(claimableAmount, token.decimals, 4)
      : "0";
  }, [
    offeringAmountInToken,
    amountReleased,
  ...
  const remaining = useMemo(() => {
    const remain = new BigNumber(offeringAmountInToken).minus(amountReleased);
    return remain.gt(0)
      ? getFullDisplayBalance(remain, token.decimals, 4)
      : "0";
  }, [token, offeringAmountInToken, amountReleased]);

  const percentage = useMemo(() => {
    const total = new BigNumber(received).plus(claimable).plus(remaining);
    const receivedPercentage = new BigNumber(received)
      .div(total)
      .times(100)
      .toNumber();
    const amountAvailablePercentage = new BigNumber(claimable)
      .div(total)
      .times(100)
      .toNumber();
    return {
  ...
  return (
    <>
      <Flex justifyContent="space-between">
        ...
        <StyleTag isPrivate={poolId === PoolIds.poolBasic}>
          {labelText}
        </StyleTag>
      </Flex>
      ...
      <WhiteCard>
        <Progress
          primaryStep={percentage.receivedPercentage}
          secondaryStep={percentage.amountAvailablePercentage}
        />
        <Flex>
