...
const MoreFromThisCollection: React.FC<
  React.PropsWithChildren<MoreFromThisCollectionProps>
> = ({
  collectionAddress,
  currentTokenName = "",
  ...
  const checkSummedCollectionAddress =
    isAddress(collectionAddress) || collectionAddress;
  ...
  const { data: collectionNfts } = useSWRImmutable<NftToken[]>(
    ...
    async () => {
      try {
        ...
        const nftsMarket = await getMarketDataForTokenIds(
          collectionAddress,
          tokenIds
        );
        ...
        console.error(
          `Failed to fetch collection NFTs for ${collectionAddress}`,
          error
        );
        return [];
      }
    ...
    return shuffle(
      allPancakeBunnyNfts
        ...
        : collectionNfts?.filter(
            (nft) => nft.name !== currentTokenName && nft.marketData?.isTradable
          )
    );
