...
export default function SwapForm() {
  ...
  const hasStableSwapAlternative = useMemo(() => {
    return stablePairs.some((stablePair) => {
      ...
      return (
        (checkSummedToken0 === inputCurrencyId ||
          checkSummedToken0 === outputCurrencyId) &&
        (checkSummedToken1 === inputCurrencyId ||
          checkSummedToken1 === outputCurrencyId)
      );
    });
        ...
        {hasStableSwapAlternative && (
          <AutoColumn>
            <Message variant="warning" my="16px">
              <MessageText>
                {t(
                  "Trade stablecoins in StableSwap with lower slippage and trading fees!"
                )}
              </MessageText>
            </Message>
          </AutoColumn>
