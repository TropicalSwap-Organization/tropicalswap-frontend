...
export function useSmartRouterContract() {
  ...
  return useContract<PancakeSwapSmartRouter>(
    SMART_ROUTER_ADDRESS[chainId],
    PancakeSwapSmartRouterABI,
    true
  );
}
...
export function computeTradePriceBreakdown(
    ...
    : ONE_HUNDRED_PERCENT.subtract(
        trade.route.pairs.reduce<Fraction>(
          (currentFee: Fraction, pair): Fraction =>
            currentFee.multiply(
              isStableSwapPair(pair)
                ? ONE_HUNDRED_PERCENT.subtract(pair.fee)
                : INPUT_FRACTION_AFTER_FEE
            ),
          ONE_HUNDRED_PERCENT
        )
  ...
  const priceImpactWithoutFeeFraction =
    trade && realizedLPFee
      ? Trade.priceImpact(trade).subtract(realizedLPFee)
      : undefined;
...
export function formatExecutionPrice(
  ...
  return inverted
    ? `${Trade.executionPrice(trade).invert().toSignificant(6)} ${
        trade.inputAmount.currency.symbol
      } / ${trade.outputAmount.currency.symbol}`
    : `${Trade.executionPrice(trade).toSignificant(6)} ${
        trade.outputAmount.currency.symbol
      } / ${trade.inputAmount.currency.symbol}`;
}
