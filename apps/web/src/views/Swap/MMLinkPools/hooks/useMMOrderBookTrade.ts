...
function involvesAddress(
  ...
  return (
    trade.route.path.some(
      (token) => token.isToken && token.address === checksummedAddress
    ) ||
    trade.route.pairs.some(
      (pair) => (pair as Pair)?.liquidityToken?.address === checksummedAddress
    )
  );
}
...
const checkOrderBookShouldRefetch = (
  ...
  const shouldRefetch = !(
    ...
    Boolean(
      rfqInputPath === rfqUserInputPath?.current &&
        rfqUserInputPath?.current !== undefined
    )
  );
...
export const useMMTrade = (
  ...
  const bestTradeWithMM = useMemo(() => {
    ...
    if (
      ...
      !outputCurrency ||
      !mmQuote ||
      !mmQuote?.message?.takerSideTokenAmount
    )
      result = null;
    else {
      // const { takerSideTokenAmount, makerSideTokenAmount } = mmQuote?.message
      const takerSideTokenAmount = mmQuote?.message?.takerSideTokenAmount ?? "";
      const makerSideTokenAmount = mmQuote?.message?.makerSideTokenAmount ?? "";
      result = parseMMTrade(
        isExactIn,
        inputCurrency,
        outputCurrency,
  ...
  const inputError = useMemo(() => {
    ...
    if (balanceIn && amountIn && balanceIn.lessThan(amountIn)) {
      result = t("Insufficient %symbol% balance", {
