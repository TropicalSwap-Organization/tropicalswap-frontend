...
const BannerPlaceHolder = styled.div<{ walletConnected: boolean }>`
  ...
  margin-top: ${({ walletConnected }) => (walletConnected ? "250px" : "0px")};
  margin-bottom: ${({ walletConnected }) =>
    walletConnected ? "-220px" : "0px"};
  ${({ theme }) => theme.mediaQueries.sm} {
    margin-top: ${({ walletConnected }) =>
      walletConnected ? "190px" : "-32px"};
    margin-bottom: 30px;
  }
  ${({ theme }) => theme.mediaQueries.md} {
    margin-top: ${({ walletConnected }) =>
      walletConnected ? "90px" : "-32px"};
    margin-bottom: ${({ walletConnected }) =>
      walletConnected ? "40px" : "30px"};
  }
  ${({ theme }) => theme.mediaQueries.lg},${({ theme }) =>
    theme.mediaQueries.md} {
    padding-top: 0;
    margin-top: ${({ walletConnected }) =>
      walletConnected ? "60px" : "-32px"};
    margin-bottom: ${({ walletConnected }) =>
      walletConnected ? "60px" : "30px"};
  }
  .swiper-slide {
    overflow: visible;
    &::-webkit-scrollbar {
...
const MultipleBanner: React.FC<React.PropsWithChildren> = () => {
  ...
  return (
    <BannerPlaceHolder
      walletConnected={Boolean(account) && chainId === ChainId.BSC}
    >
      <div
        style={{
          position: "absolute",
      ...
      >
        <div className="text-[#FDE736] mb-4">Launchpad</div>
        <div className="mb-4" style={{ color: "white" }}>
          $PAPPLE IDO SOON!
        </div>
        {/* <Button style={{ background: '#FFBF00', color: "#400C8B" }}>Buy in presale</Button> */}
        <Button
        ...
        >
          <Text
            color="#400C8B"
            className="font-mitr mr-2"
            bold
            fontSize="16px"
            mr="4px"
            style={{ color: "#400C8B", marginRight: "4px" }}
          >
            Buy in presale
          </Text>
        ...
        <img
          ...
          alt="launchpad"
          className="absolute right-40 h-[17rem] w-[15rem]"
        />
        {/* Buy in presale */}
      </div>
      {/* <StyledSwiper
