...
const WrongNetworkSelect = ({ switchNetwork, chainId }) => {
  ...
  /*   const localChainId = useLocalNetworkChain() || ChainId.MANTLE_TESTNET; */
  const localChainId = useLocalNetworkChain() || ChainId.MANTLE;
  const [, setSessionChainId] = useSessionChainId();

  const localChainName =
    chains.find((c) => c.id === localChainId)?.name ?? "mnt";
    ...
    >
      ...
      > */}
        {isLoading ? (
          ...
          <>
            <span
              className="font-mitr lg:text-[18px] lg:leading-[25px] text-[12px] leading-[18px]"
              style={{ color: "white" }}
            >
              Built on
            </span>
