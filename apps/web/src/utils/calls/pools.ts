...
export const getActivePools = async (block?: number) => {
  const eligiblePools = poolsConfig
    ...
    .filter(
      (pool) => pool.isFinished === false || pool.isFinished === undefined
    );
  ...
  const calls = !block
    ? [...startBlockCalls, ...endBlockCalls, blockCall]
  ...
  return eligiblePools.reduce((accum, poolCheck, index) => {
    const startBlock = startBlocks[index]
      ? new BigNumber(startBlocks[index])
      : null;
