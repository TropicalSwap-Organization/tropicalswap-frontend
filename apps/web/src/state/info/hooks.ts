...
import {
  fetchAllTokenData,
  fetchAllTokenDataByAddresses,
} from "state/info/queries/tokens/tokenData";
...
export const usePoolChartDataSWR = (
  address: string
...
export const usePoolTransactionsSWR = (
  address: string
...
const fetcher = (
  ...
  for (let i = 0; i < times; i++) {
    addressGroup.push(
      addresses.slice(i * graphPerPage, (i + 1) * graphPerPage)
    );
  }
  return Promise.all(
    addressGroup.map((d) => fetchAllTokenDataByAddresses(chainName, blocks, d))
  );
};
...
export const useTokenChartDataSWR = (
  address: string
): ChartEntry[] | undefined => {
  const chainName = useGetChainName();
  const type = checkIsStableSwap() ? "stableSwap" : "swap";
  const { data } = useSWRImmutable(
    address &&
      chainName && [`info/token/chartData/${address}/${type}`, chainName],
...
export const useTokenPriceDataSWR = (
  ...
  const startTimestamp = getUnixTime(
    startOfHour(sub(utcCurrentTime, timeWindow))
  );
  const chainName = useGetChainName();
  const type = checkIsStableSwap() ? "stableSwap" : "swap";
  const { data } = useSWRImmutable(
...
export const useTokenTransactionsSWR = (
  address: string
): Transaction[] | undefined => {
  const chainName = useGetChainName();
  const type = checkIsStableSwap() ? "stableSwap" : "swap";
...
export const useGetChainName = () => {
  ...
  const getChain = useCallback(() => {
    ...
    if (pathname.includes("mnt") || query.chain === "mnt") return "MNT";
    return "BSC";
  }, [pathname, query]);
...
export const useStableSwapTopPoolsAPR = (
  addresses: string[]
  ...
  const { data } = useSWRImmutable<BigNumber[]>(
    isStableSwap &&
      addresses?.length > 0 && [`info/pool/stableAPRs/Addresses/`, chainName],
