...
export const mapDayData = (
  tokenDayData: TokenDayData | PancakeDayData
): ChartEntry => ({
...
type OverviewFetchFn = (
  chianName: MultiChainName,
  skip: number
) => Promise<{ data?: ChartEntry[]; error: boolean }>;
...
export const fetchChartData = async (
  ...
  while (!allFound && !error) {
    ...
    const { data, error: fetchError } = await getEntityDayDatas(
      chainName,
      skip
    );
    skip += 1000;
  ...
  const availableDays = Object.keys(formattedDayDatas).map((dayOrdinal) =>
    parseInt(dayOrdinal, 10)
  );
...
export async function getPairTokenMap(
  poolAddresses: string[],
  chainName: "ETH" | "BSC" | "MNT"
) {
  ...
  const pairTokenResults = rawPairTokenResults
    ? rawPairTokenResults.map((rawPairTokenResult) =>
        rawPairTokenResult[0].toLowerCase()
      )
    : [];
  const pairTokenMap: {
    [tokenAddress: string]: { token0: string; token1: string };
  } = chunk(pairTokenResults, 2).reduce((acc, tokenResult, index) => {
    return {
      ...acc,
      [poolAddresses[index].toLowerCase()]: {
        token0: tokenResult[0],
        token1: tokenResult[1],
      },
    };
  }, {});
  let rawTokenResults;
  ...
  try {
    rawTokenResults = await multicallv2({
      ...
      chainId:
        chainName === "ETH"
          ? ChainId.ETHEREUM
          : ChainId.BSC || ChainId.BSC_TESTNET || ChainId.MANTLE,
      /*      : ChainId.BSC || ChainId.BSC_TESTNET || ChainId.MANTLE_TESTNET, */
    });
  } catch (error) {
    console.info("Error fetching tokenIds from pair");
  }
  const tokenResults = rawTokenResults
    ? rawTokenResults.map((rawTokenResult) => rawTokenResult[0])
    : [];
  const tokenMap = chunk(tokenResults, 2).reduce((acc, tokenResult, index) => {
    return {
      ...acc,
      [uniquePairTokenResults[index].toLowerCase()]: {
        name: tokenResult[0],
        symbol: tokenResult[1],
      },
    };
  }, {});
