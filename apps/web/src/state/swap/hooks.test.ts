...
describe("hooks", () => {
  describe("#queryParametersToSwapState", () => {
    ...
    test("does not duplicate BNB for invalid output token", () => {
      expect(
        queryParametersToSwapState(parse("outputCurrency=invalid"), "BNB")
      ).toEqual({
    ...
    test("valid recipient", () => {
      expect(
        queryParametersToSwapState(
          parse(
            "outputCurrency=BNB&exactAmount=20.5&recipient=0x0fF2D1eFd7A57B7562b2bf27F3f37899dB27F4a5"
          ),
          "BNB"
        )
      ).toEqual({
...
describe("#useDerivedSwapInfo", () => {
  it("should show Login Error", async () => {
    ...
    mockAccount.mockReturnValue({
      address: "0x33edFBc4934baACc78f4d317bc07639119dd3e78",
    });
    rerender();
